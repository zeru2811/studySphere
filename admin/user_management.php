                    ...
                    <div class="flex items-center">
                        <div class="bg-purple-500 text-white rounded-full w-10 h-10 flex items-center justify-center">
                            <?php if (!empty($row['profile_photo'])): ?>
                                <img src="../uploads/profiles/<?= htmlspecialchars($row['profile_photo']) ?>" alt="Profile Photo" class="w-8 h-8 rounded-full">
                            <?php else: ?>
        ...
        <div class=" header-content flex flex-col lg:flex-row lg:justify-between lg:items-center">
            ...
            <div class="header-actions flex items-center mt-4 lg:mt-0">
                ...
                <div class="relative inline-block text-left">
                    <?php 
                    $sql = "Select profile_photo, role_id, name from users where id = ?";
                    $stmt = $mysqli->prepare($sql); 
                    $stmt->bind_param("i", $_SESSION['id']);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $user = $result->fetch_assoc();
                    //if role_id is 1 "Admin", 2 "Teacher"
                    $role = $user['role_id'];
                    if ($role == 1) {
                        $roleName = "Admin";
                    } elseif ($role == 2) {
                    ...
                    $profilePhoto = !empty($user['profile_photo']) ? '../uploads/profiles/' . htmlspecialchars($user['profile_photo'], ENT_QUOTES, 'UTF-8') : '../img/image.png';
                    $stmt->close();
                    ?>
                    <!-- Button -->
                    <button id="userMenuBtn" class="flex items-center focus:outline-none">
                        ...
                        <div class="text-sm font-medium text-gray-800">
                            <span class="ml-2 text-gray-700 hidden sm:inline"><?= $name ?></span><br>
                            <span class="ml-2 text-gray-700 hidden sm:inline" style="font-size:x-small;"><?= $roleName ?></span>
                        </div>
                    </button>
                            ...
                            <tr>
                                <td class="py-4 px-6 ">
                                    <div class="flex items-center">
                                        <div class="bg-purple-500 text-white rounded-full w-10 h-10 flex items-center justify-center">
                                            <?php if (!empty($row['profile_photo'])): ?>
                                                <img src="../uploads/profiles/<?= htmlspecialchars($row['profile_photo']) ?>" alt="Profile Photo" class="w-9 h-9 rounded-full object-cover">
                                            <?php else: ?>
                                                <span><?= htmlspecialchars(substr($row['name'], 0, 2)) ?></span>
                                            <?php endif; ?>
                                        </div>
